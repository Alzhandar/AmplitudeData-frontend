...
import { formatDateTime, formatDate, formatMoney, formatNumber } from "@/features/common/utils/format";
...
} from "@/features/guest-profile/types";
...
const CRYSTAL_TYPE_LABELS: Record<number, string> = {
  0: "Подарок",
  1: "Игра",
  2: "День рождения",
  3: "Покупка",
...
function WarningBanner({
  warnings, tab, dismissed, onDismiss,
}: {
  warnings: string[];
  tab: Tab;
      ...
      <ul className="mt-1 space-y-1">
        {visible.map((w) => (
          <li key={w} className="flex items-start justify-between gap-2">
            ...
            <button type="button" onClick={() => onDismiss(w)} className="mt-0.5 shrink-0 rounded text-amber-600 hover:text-amber-900" aria-label="Закрыть">
...
function TabBar({ active, onSelect, result }: { active: Tab; onSelect: (t: Tab) => void; result: GuestProfileResponse }) {
      ...
      {TABS.map((t) => {
        ...
        return (
          ...
          >
            {t.label}
            {count !== null && (
              <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-semibold leading-none ${isActive ? "bg-white/25 text-white" : "bg-slate-100 text-slate-500"}`}>
                {formatNumber(count)}
              </span>
            )}
          </button>
...
function PurchaseDetailModal({ open, onClose, order }: { open: boolean; onClose: () => void; order: Order | null }) {
  if (!order) return null;
  return (
    <Modal open={open} onClose={onClose} title={`Заказ #${order.id} · ${formatDateTime(order.c_created)}`} size="lg"
      footer={<Button variant="secondary" size="sm" onClick={onClose}>Закрыть</Button>}
    >
      <div className="space-y-5 text-sm">
        <dl className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {([
            ["Парк", order.park_name || "—"],
            ["Сумма", formatMoney(order.fact_sum)],
            ["Тип чека", order.check_type === 1 ? "Продажа" : order.check_type === -1 ? "Возврат" : "—"],
            ["Статус", ORDER_STATUS_LABELS[order.status] ?? "—"],
            ["Официант", order.waiter_name || "—"],
            ["Чек №", order.receipt_number != null ? String(order.receipt_number) : "—"],
        ...
        <div>
          <p className="mb-2 text-sm font-semibold text-slate-700">Товары ({order.items.length})</p>
          {order.items.length === 0 ? <EmptyState text="Позиции не найдены" /> : (
        ...
        <div>
          <p className="mb-2 text-sm font-semibold text-slate-700">Оплаты ({order.payments.length})</p>
          {order.payments.length === 0 ? <EmptyState text="Оплаты не найдены" /> : (
            <div className="overflow-auto rounded-lg border border-slate-200">
              <table className="min-w-full text-xs">
                <thead className="bg-slate-50">
                  <tr className="text-left text-[10px] uppercase tracking-wide text-slate-400">
                ...
                <tbody>
                  {order.payments.map((p, i) => (
                    <tr key={p.id ?? i} className="border-t border-slate-100">
                      <td className="px-3 py-2">{p.payment_type_name || "—"}</td>
...
function OverviewTab({ result, warnings, dismissed, onDismiss }: { result: GuestProfileResponse; warnings: string[]; dismissed: Set<string>; onDismiss: (w: string) => void }) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {cb.burn_date && (
            <p className="mt-1 text-xs text-slate-500">Сгорает {formatDate(cb.burn_date)}: {formatMoney(cb.burn_sum)}</p>
          )}
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
...
function PurchasesTab({ result, warnings, dismissed, onDismiss }: { result: GuestProfileResponse; warnings: string[]; dismissed: Set<string>; onDismiss: (w: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {results.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><EmptyState text="Покупок за выбранный период не найдено" /></div>
      ) : (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {results.map((order, idx) => {
                  const isReturn = order.check_type === -1;
                  return (
                    <tr key={order.id ?? idx} className="border-t border-slate-100 hover:bg-slate-50/50">
                      <td className="px-4 py-3 text-slate-700">{formatDateTime(order.c_created)}</td>
                      <td className="px-4 py-3 text-slate-700">{order.park_name || "—"}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${isReturn ? "bg-rose-50 text-rose-700" : "bg-slate-100 text-slate-600"}`}>
                          {isReturn ? "Возврат" : "Продажа"}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right font-medium text-slate-900">{formatMoney(order.fact_sum)}</td>
                      <td className="px-4 py-3 text-right text-slate-500">{order.items.length}</td>
                      <td className="px-4 py-3">
...
function CashbackTab({ result, warnings, dismissed, onDismiss }: { result: GuestProfileResponse; warnings: string[]; dismissed: Set<string>; onDismiss: (w: string) => void }) {
  const cb = result.balances.cashback;
  ...
  return (
    <div className="space-y-4">
      ...
      {results.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><EmptyState text="История операций не найдена" /></div>
      ) : (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="overflow-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {results.map((row: CashbackRecord, idx) => {
                  const isAccrual = row.type === 1;
                  return (
                    <tr key={row.id ?? idx} className="border-t border-slate-100 hover:bg-slate-50/50">
                      <td className="px-4 py-3 text-slate-700">{formatDateTime(row.transaction_date)}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${isAccrual ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>
                          {isAccrual ? "Начисление" : "Списание"}
                        </span>
                      </td>
                      <td className={`px-4 py-3 text-right font-medium ${isAccrual ? "text-emerald-700" : "text-rose-700"}`}>
                        {isAccrual ? "+" : ""}{formatMoney(row.amount)}
                      </td>
...
function CrystalsTab({ result, warnings, dismissed, onDismiss }: { result: GuestProfileResponse; warnings: string[]; dismissed: Set<string>; onDismiss: (w: string) => void }) {
  const cr = result.balances.crystals;
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="overflow-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {results.map((row: CrystalRecord, idx) => {
                    ...
                    <tr key={row.id ?? idx} className="border-t border-slate-100 hover:bg-slate-50/50">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">
                          {CRYSTAL_TYPE_LABELS[row.type] ?? "—"}
                        </span>
                      </td>
                      <td className={`px-4 py-3 text-right font-medium ${isPositive ? "text-emerald-700" : "text-rose-700"}`}>
                        {isPositive ? "+" : ""}{formatNumber(row.amount)}
                      </td>
                      <td className="px-4 py-3 text-slate-500">{row.description_ru || "—"}</td>
                    </tr>
                  );
                })}
              </tbody>
...
function MobileTab({ result, warnings, dismissed, onDismiss }: { result: GuestProfileResponse; warnings: string[]; dismissed: Set<string>; onDismiss: (w: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {results.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><EmptyState text="Событий мобильного приложения не найдено" /></div>
      ) : (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="overflow-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {results.map((row: MobileActivityRecord, idx) => {
                  const deviceLabel = [row.device_brand, row.device_model].filter(Boolean).join(" ") || row.device_id || "—";
                  return (
                    <tr key={`${row.event_time ?? "x"}-${row.event_type}-${row.device_id}-${idx}`} className="border-t border-slate-100 hover:bg-slate-50/50">
                      ...
                      <td className="px-4 py-3">
                        {row.platform ? (
                          <span className="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">{row.platform}</span>
                        ) : "—"}
                      </td>
...
export default function GuestProfilePage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(translateErrorMessage(err instanceof Error ? err.message : "Не удалось получить информацию о госте"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="space-y-5">
        ...
        {result && !loading && (
          <>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-lg font-semibold text-slate-900">{result.guest.name || "Имя не указано"}</h3>
                    <GuestStatusBadge isBlocked={result.guest.status.is_blocked} label={result.guest.status.label} />
                  </div>
                  <p className="mt-0.5 text-sm text-slate-500">#{result.guest.id} · {result.phone}</p>
                </div>
              </div>
            </section>
            ...
            {activeTab === "overview" && <OverviewTab result={result} warnings={result.warnings} dismissed={dismissed} onDismiss={dismissWarning} />}
            {activeTab === "purchases" && <PurchasesTab result={result} warnings={result.warnings} dismissed={dismissed} onDismiss={dismissWarning} />}
            {activeTab === "cashback" && <CashbackTab result={result} warnings={result.warnings} dismissed={dismissed} onDismiss={dismissWarning} />}
            {activeTab === "crystals" && <CrystalsTab result={result} warnings={result.warnings} dismissed={dismissed} onDismiss={dismissWarning} />}
            {activeTab === "mobile" && <MobileTab result={result} warnings={result.warnings} dismissed={dismissed} onDismiss={dismissWarning} />}
          </>
        )}
      </div>
    </AppShell>
